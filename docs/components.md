...
rskj clients are served in rskj's own format, which they are told apart by:
their chunk request carries three elements where rustock's carries four. The
encoder is checked byte-for-byte against vectors generated by running rskj's
code, which also established that for the same keys rustock's trie is identical
